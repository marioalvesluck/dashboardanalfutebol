# Lista de nomes para substituição
times_pt = ['Boavista FC', 'FC Porto', 'Sporting Clube de Portugal', 'GD Estoril Praia', 'Portimonense SC',
            'Casa Pia AC', 'Sporting Clube de Braga', 'SC Farense', 'Gil Vicente FC', 'GD Chaves',
            'Sport Lisboa e Benfica', 'FC Famalicão', 'CF Estrela da Amadora', 'FC Arouca', 'FC Vizela',
            'Moreirense FC', 'Vitória SC', 'Rio Ave FC']

times_it = [
    'FC Internazionale Milano', 'Genoa CFC', 'Juventus FC', 'AS Roma',
    'AC Milan', 'US Sassuolo Calcio', 'Hellas Verona FC', 'US Salernitana 1919', 'Udinese Calcio', 'Bologna FC 1909',
    'Empoli FC', 'Atalanta BC', 'US Lecce', 'SS Lazio', 'Frosinone Calcio',
    'ACF Fiorentina', 'Torino FC', 'SSC Napoli', 'AC Monza',
    'Cagliari Calcio', 'US Cremonese', 'Parma Calcio 1913', 'Spezia Calcio', 'AC Reggiana'
]

times_esp = [

    'UD Las Palmas', 'FC Barcelona', 'Sevilla FC', 'Athletic Club', 'CA Osasuna', 'UD Almería', 'Girona FC',
    'Club Atlético de Madrid', 'RC Celta de Vigo', 'Real Betis Balompié', 'Real Madrid CF', 'RCD Mallorca',
    'Granada CF', 'Cádiz CF', 'Valencia CF', 'Villarreal CF', 'Real Sociedad de Fútbol', 'Deportivo Alavés',
    'Getafe CF', 'Rayo Vallecano de Madrid', 'Orihuela CF',  'CD Cayon',  'Sestao River Club',
    'UD Melilla', 'SD Eibar', 'Linares Deportivo', 'Elche CF',
    'Racing Club de Ferrol', 'CD Leganés', 'Arandina CF', 'Atlético Astorga', 'CD Tudelano', 'CD Valle de Egüés',
    'CF Villanovense',  'CD Lugo', 'CD Mirandés', 'Levante UD',
    'SD Amorebieta', 'Málaga CF', 'CD Eldense', 'Terrassa FC',
    'CD Lugo',  'RCD Espanyol de Barcelona',
    'Elche CF',  'SD Huesca',
    'Burgos CF', 'CD Castellón',
    'Unionistas de Salamanca CF',
    'FC Cartagena', 'UD Barbastro', 'CD Tenerife'
]

times_ig = [
    'West Ham United FC', 'Brighton & Hove Albion FC', 'Liverpool FC', 'Newcastle United FC', 'Tottenham Hotspur FC',
    'AFC Bournemouth', 'Fulham FC', 'Arsenal FC', 'Nottingham Forest FC', 'Manchester United FC', 'Aston Villa FC',
    'Burnley FC', 'Crystal Palace FC', 'Brentford FC', 'Wolverhampton Wanderers FC', 'Everton FC',
    'Manchester City FC', 'Sheffield United FC', 'Luton Town FC', 'Chelsea FC',

    "Sunderland AFC", "Maidstone United FC", "AFC Wimbledon", "Millwall FC", "Coventry City FC",
    "Blackpool FC", "Stockport County FC", "Port Vale FC",
    "Scarborough Athletic FC", "Barnet FC", "Crewe Alexandra FC", "Norwich City FC", "Blackburn Rovers FC",
    "Queens Park Rangers FC", "Newport County AFC", "Plymouth Argyle FC", "Watford FC", "Hull City AFC",
    "Gillingham FC", "Southampton FC", "Stoke City FC",  "Swansea City AFC", "Sheffield Wednesday FC",
    "Middlesbrough FC", 'Bolton Wanderes FC', 'Huddersfield Town AFC', 'Peterborough United FC', 'Leeds United FC', 'Shrewsbury Town FC', 'Wrexham AFC', 'West Bromwich Albion FC', 'Aldershot Town FC'
    "Stevenage FC", "Ipswich Town FC", "Leicester City FC", "Oxford United FC",
    "Rotherham United FC",  "Forest Green Rovers FC", "Aldershot Town FC",
    "Stevenage FC", "Bristol Rovers FC", "Bristol City FC", 'Wigan Athletic FC',
    "Cambridge United FC", "Eastleigh FC", "Sutton United FC", "Chesterfield FC",
    "Birmingham City FC", "Walsall FC", "Preston North End FC",
    "Morecambe FC", "Cardiff City FC", 'Bolton Wanderers FC', 'Cambridge United FC'
]

times_al = [
    'Bayer 04 Leverkusen', 'VfL Bochum 1848', '1. FC Heidenheim 1846', 'SC Freiburg', 'Eintracht Frankfurt',
    'Borussia Mönchengladbach', 'VfL Wolfsburg', 'FC Bayern München', 'VfB Stuttgart', 'FC Augsburg',
    '1. FC Union Berlin', '1. FC Köln', 'Borussia Dortmund', '1. FSV Mainz 05', 'TSG 1899 Hoffenheim',
    'SV Darmstadt 98', 'SV Werder Bremen', 'RB Leipzig'
]

times_fr = [
    'Clermont Foot 63', 'Stade Rennais FC 1901', 'Toulouse FC', 'AS Monaco FC', 'Montpellier HSC',
    'Olympique de Marseille', 'OGC Nice', 'Racing Club de Lens', 'Olympique Lyonnais',
    'FC Nantes', 'Paris Saint-Germain FC', 'RC Strasbourg Alsace', 'Stade Brestois 29',
    'FC Lorient', 'Stade de Reims', 'Lille OSC', 'FC Metz', 'Le Havre AC'
]

times_hol = [
    'AZ', 'PSV', 'AFC Ajax', 'PEC Zwolle', 'Heracles Almelo', 'Feyenoord Rotterdam',
    'Sparta Rotterdam', 'FC Twente \'65', 'Almere City FC', 'SBV Vitesse', 'SC Heerenveen',
    'FC Volendam', 'SBV Excelsior', 'Go Ahead Eagles', 'FC Utrecht', 'RKC Waalwijk', 'NEC',
    'Fortuna Sittard'
]


teams_cl = [
    'Royal Antwerp FC', 'FC Barcelona',
    'Newcastle United FC', 'AC Milan',
    'Celtic FC', 'Feyenoord Rotterdam',
    'FC Porto', 'FK Shakhtar Donetsk',
    'Borussia Dortmund', 'Paris Saint-Germain FC',
    'Club Atlético de Madrid', 'SS Lazio',
    'RB Leipzig', 'BSC Young Boys',
    'FK Crvena Zvezda', 'Manchester City FC',
    'FC København', 'Galatasaray SK',
    'FC Red Bull Salzburg', 'Sport Lisboa e Benfica',
    'FC Internazionale Milano', 'Real Sociedad de Fútbol',
    'Manchester United FC', 'FC Bayern München',
    'SSC Napoli', 'Sporting Clube de Braga',
    '1. FC Union Berlin', 'Real Madrid CF',
    'Racing Club de Lens', 'Sevilla FC',
    'PSV', 'Arsenal FC', 'Arsenal FC', 'PSV'
]


# Lista de nomes para substituição
times = ['Boavista FC', 'FC Porto', 'Sporting Clube de Portugal', 'GD Estoril Praia', 'Portimonense SC',
         'Casa Pia AC', 'Sporting Clube de Braga', 'SC Farense', 'Gil Vicente FC', 'GD Chaves',
         'Sport Lisboa e Benfica', 'FC Famalicão', 'CF Estrela da Amadora', 'FC Arouca', 'FC Vizela',
         'Moreirense FC', 'Vitória SC', 'Rio Ave FC'


         'FC Internazionale Milano', 'Genoa CFC', 'Juventus FC', 'AS Roma',
         'AC Milan', 'US Sassuolo Calcio', 'Hellas Verona FC', 'US Salernitana 1919', 'Udinese Calcio', 'Bologna FC 1909',
         'Empoli FC', 'Atalanta BC', 'US Lecce', 'SS Lazio', 'Frosinone Calcio',
         'ACF Fiorentina', 'Torino FC', 'SSC Napoli', 'AC Monza',
         'Cagliari Calcio', 'US Cremonese', 'Parma Calcio 1913', 'Spezia Calcio', 'AC Reggiana'


         'UD Las Palmas', 'FC Barcelona', 'Sevilla FC', 'Athletic Club', 'CA Osasuna', 'UD Almería', 'Girona FC',
         'Club Atlético de Madrid', 'RC Celta de Vigo', 'Real Betis Balompié', 'Real Madrid CF', 'RCD Mallorca',
         'Granada CF', 'Cádiz CF', 'Valencia CF', 'Villarreal CF', 'Real Sociedad de Fútbol', 'Deportivo Alavés',
         'Getafe CF', 'Rayo Vallecano de Madrid', 'Orihuela CF',  'CD Cayon',  'Sestao River Club',
         'UD Melilla', 'SD Eibar', 'Linares Deportivo', 'Elche CF',
         'Racing Club de Ferrol', 'CD Leganés', 'Arandina CF', 'Atlético Astorga', 'CD Tudelano', 'CD Valle de Egüés',
         'CF Villanovense',  'CD Lugo', 'CD Mirandés', 'Levante UD',
         'SD Amorebieta', 'Málaga CF', 'CD Eldense', 'Terrassa FC',
         'CD Lugo',  'RCD Espanyol de Barcelona',
         'Elche CF',  'SD Huesca',
         'Burgos CF', 'CD Castellón',
         'Unionistas de Salamanca CF',
         'FC Cartagena', 'UD Barbastro', 'CD Tenerife'

         'West Ham United FC', 'Brighton & Hove Albion FC', 'Liverpool FC', 'Newcastle United FC', 'Tottenham Hotspur FC',
         'AFC Bournemouth', 'Fulham FC', 'Arsenal FC', 'Nottingham Forest FC', 'Manchester United FC', 'Aston Villa FC',
         'Burnley FC', 'Crystal Palace FC', 'Brentford FC', 'Wolverhampton Wanderers FC', 'Everton FC',
         'Manchester City FC', 'Sheffield United FC', 'Luton Town FC', 'Chelsea FC',

         "Sunderland AFC", "Maidstone United FC", "AFC Wimbledon", "Millwall FC", "Coventry City FC",
         "Blackpool FC", "Stockport County FC", "Port Vale FC",
         "Scarborough Athletic FC", "Barnet FC", "Crewe Alexandra FC", "Norwich City FC", "Blackburn Rovers FC",
         "Queens Park Rangers FC", "Newport County AFC", "Plymouth Argyle FC", "Watford FC", "Hull City AFC",
         "Gillingham FC", "Southampton FC", "Stoke City FC",  "Swansea City AFC", "Sheffield Wednesday FC",
         "Middlesbrough FC", 'Bolton Wanderes FC', 'Huddersfield Town AFC', 'Peterborough United FC', 'Leeds United FC', 'Shrewsbury Town FC', 'Wrexham AFC', 'West Bromwich Albion FC', 'Aldershot Town FC'
         "Stevenage FC", "Ipswich Town FC", "Leicester City FC", "Oxford United FC",
         "Rotherham United FC",  "Forest Green Rovers FC", "Aldershot Town FC",
         "Stevenage FC", "Bristol Rovers FC", "Bristol City FC", 'Wigan Athletic FC',
         "Cambridge United FC", "Eastleigh FC", "Sutton United FC", "Chesterfield FC",
         "Birmingham City FC", "Walsall FC", "Preston North End FC",
         "Morecambe FC", "Cardiff City FC", 'Bolton Wanderers FC', 'Cambridge United FC'

         'Bayer 04 Leverkusen', 'VfL Bochum 1848', '1. FC Heidenheim 1846', 'SC Freiburg', 'Eintracht Frankfurt',
         'Borussia Mönchengladbach', 'VfL Wolfsburg', 'FC Bayern München', 'VfB Stuttgart', 'FC Augsburg',
         '1. FC Union Berlin', '1. FC Köln', 'Borussia Dortmund', '1. FSV Mainz 05', 'TSG 1899 Hoffenheim',
         'SV Darmstadt 98', 'SV Werder Bremen', 'RB Leipzig'

         'Clermont Foot 63', 'Stade Rennais FC 1901', 'Toulouse FC', 'AS Monaco FC', 'Montpellier HSC',
         'Olympique de Marseille', 'OGC Nice', 'Racing Club de Lens', 'Olympique Lyonnais',
         'FC Nantes', 'Paris Saint-Germain FC', 'RC Strasbourg Alsace', 'Stade Brestois 29',
         'FC Lorient', 'Stade de Reims', 'Lille OSC', 'FC Metz', 'Le Havre AC'

         'AZ', 'PSV', 'AFC Ajax', 'PEC Zwolle', 'Heracles Almelo', 'Feyenoord Rotterdam',
         'Sparta Rotterdam', 'FC Twente \'65', 'Almere City FC', 'SBV Vitesse', 'SC Heerenveen',
         'FC Volendam', 'SBV Excelsior', 'Go Ahead Eagles', 'FC Utrecht', 'RKC Waalwijk', 'NEC',
         'Fortuna Sittard'

         'Royal Antwerp FC', 'FC Barcelona',
         'Newcastle United FC', 'AC Milan',
         'Celtic FC', 'Feyenoord Rotterdam',
         'FC Porto', 'FK Shakhtar Donetsk',
         'Borussia Dortmund', 'Paris Saint-Germain FC',
         'Club Atlético de Madrid', 'SS Lazio',
         'RB Leipzig', 'BSC Young Boys',
         'FK Crvena Zvezda', 'Manchester City FC',
         'FC København', 'Galatasaray SK',
         'FC Red Bull Salzburg', 'Sport Lisboa e Benfica',
         'FC Internazionale Milano', 'Real Sociedad de Fútbol',
         'Manchester United FC', 'FC Bayern München',
         'SSC Napoli', 'Sporting Clube de Braga',
         '1. FC Union Berlin', 'Real Madrid CF',
         'Racing Club de Lens', 'Sevilla FC',
         'PSV', 'Arsenal FC', 'Arsenal FC', 'PSV'
         ]
